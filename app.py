...
def main():
    ...
    st.logo(
        Image.open("chat_icons/iammoai.png"),
        icon_image=Image.open("chat_icons/bot_image.png")
    )
    st.sidebar.header("Chat Sessions")
    chat_sessions = ["New session"] + get_all_chat_history_ids()

    index = chat_sessions.index(st.session_state.session_index_tracker)
    st.sidebar.selectbox("Select a chat session", chat_sessions, key="session_key", index=index)
    clear_cache_col, delete_chat_col = st.sidebar.columns([3, 5])
    # delete_chat_col = st.sidebar.columns([3, 1])[1]
    delete_chat_col.button("Delete Chat Session", on_click=delete_chat_session_history, use_container_width=True)
    clear_cache_col.button("Clear Cache", on_click=clear_cache, use_container_width=True)

    # record_title = "Record Audio" 
    # record_title = st.sidebar.columns(2) 
    st.sidebar.header("",divider=True)
    st.sidebar.subheader("Voice Chat")
    voice_rec_col, pdf_toggle_col = st.sidebar.columns([3,2])
    # pdf_toggle_col = st.sidebar.columns(2)[1]
    pdf_toggle_col.toggle("PDF Chat", key="pdf_chat", value=False, on_change=clear_cache)
    
    with voice_rec_col:
        voice_recording=mic_recorder(start_prompt="Start recording", stop_prompt="Stop recording", just_once=True, use_container_width=True)
    ...
    if (st.session_state.session_key != "New session") != (st.session_state.new_session_key != None):
        with chat_container:
